...
function ns.GSC(cash, colorblind)
    ...
    if colorblind or GetCVarBool("colorblindMode") then
        ...
        elseif s > 0 then
            return string.format(SC, s, c)
        else
            return string.format(COPPER, c)
        end
    else
        if g > 0 and s == 0 and c == 0 then
            return string.format(GOLD, g2)
        elseif g > 99999 then
            return string.format(GOLD, g2)
        elseif g > 0 and c == 0 then
            return string.format(GS, g2, s)
        elseif g > 0 then
            return string.format(GSC, g2, s, c)
        elseif s > 0 and c == 0 then
            return string.format(GS, 0, s)
        elseif s > 0 then
            return string.format(SC, s, c)
        else
            return string.format(COPPER, c)
...
function ns.GS(cash)
    ...
    if cash > 999999 then
        ...
        return ns.GSC(floor(cash / 100) * 100)
    end
end
